...
export default function ShareButtons({ title, url }: ShareButtonsProps) {
  ...
  const shareToInstagram = async () => {
    ...
    showNotification('Link copied! Share it on Instagram');
  };
  ...
  const copyLink = async () => {
    ...
    if (success) {
      ...
      const copyText = window.prompt('Copy this link manually:', shareUrl);
      if (copyText !== null) {
        showNotification('Link shown for manual copying!');
      }
    }
  };
  ...
  return (
    <div className="mt-12 pt-8 border-t border-gray-200">
      <h3 className="text-2xl font-bold text-main-dark mb-6">Share This Post</h3>
      <div className="flex flex-wrap gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-blue-700 text-white rounded-lg hover:bg-blue-800 transition-colors duration-300"
        >
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
